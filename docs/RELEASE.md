...
3. Run `npm ci && npm run release:check:dry-run`. This explicit pre-tag mode
   validates the expected tag value while retaining all package checks.
4. Set `VERSION=$(node -p "require('./package.json').version")`.
...
6. Create the annotated tag with `git tag -a "v$VERSION" -m "Release v$VERSION"`
   and run `npm run release:check` to verify that it resolves to the current
   commit. Push only that tag with `git push origin "v$VERSION"`.
