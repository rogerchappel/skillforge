...
export async function lintSkill(dir: string): Promise<Diagnostic[]> {
  ...
  if (!manifest.activation?.antiExamples?.length) diagnostics.push(warn('activation.anti-examples', 'Add anti-examples so agents know when not to use this skill.'));
  ...
  for (const file of manifest.files ?? []) {
    ...
    if (file === 'SKILL.md') requireSkillSections(text, diagnostics, file);
  }
  return diagnostics;
}

function requireSkillSections(text: string, diagnostics: Diagnostic[], file: string): void {
  const requiredSections = [
    ['markdown.when-to-use', /##\s+When to use/i, 'Describe when agents should activate the skill.'],
    ['markdown.workflow', /##\s+(Workflow|Procedure|Steps)/i, 'Describe the repeatable workflow agents should follow.'],
    ['markdown.safety', /##\s+(Safety|Boundaries|Approval)/i, 'Document side-effect boundaries and approval requirements.'],
    ['markdown.verification', /##\s+(Verification|Validation|Checks)/i, 'Document how agents verify completion.']
  ] as const;
  for (const [code, pattern, message] of requiredSections) {
    if (!pattern.test(text)) diagnostics.push(warn(code, `${message} Missing section in ${file}.`, file));
  }
}
